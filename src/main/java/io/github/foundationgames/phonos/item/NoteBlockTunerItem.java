...
public class NoteBlockTunerItem extends Item {
    ...
    private static void playNote(SoundEvent sound, int note, World world, BlockPos pos) {
        if(!world.isClient()) for(ServerPlayerEntity player : ((ServerWorld)world).getPlayers()) player.networkHandler.sendPacket(new PlaySoundS2CPacket(sound, SoundCategory.BLOCKS, pos.getX()+0.5, pos.getY()+0.5, pos.getZ()+0.5,  1.5f, (float)Math.pow(2.0D, (double)(note - 12) / 12.0D), world.getRandom().nextLong()));
    }
