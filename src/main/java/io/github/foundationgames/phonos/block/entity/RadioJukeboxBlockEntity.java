...
public class RadioJukeboxBlockEntity extends BlockEntity
		implements ExtendedScreenHandlerFactory, SidedInventory, BlockEntityClientSerializable {
	...
	private void playSong(int slot) { //FIXME: There is an issue in this function.
	...
	public static void tick(World world, BlockPos pos, BlockState state, RadioJukeboxBlockEntity self) {
		...
		boolean s = world.getBlockState(pos).getBlock() instanceof RadioJukeboxBlock
				&& world.getBlockState(pos).get(RadioJukeboxBlock.PLAYING);
	...
	public void setDuration(int slot, int duration) {
		if (slot == 0)
			disc1Duration = duration;
		else if (slot == 1)
			disc2Duration = duration;
		else if (slot == 2)
			disc3Duration = duration;
		else if (slot == 3)
			disc4Duration = duration;
		else if (slot == 4)
			disc5Duration = duration;
		else if (slot == 5)
			disc6Duration = duration;
	}
	...
	public static void registerServerPackets() {
		ServerPlayNetworking.registerGlobalReceiver(Phonos.id("update_radio_jukebox"),
